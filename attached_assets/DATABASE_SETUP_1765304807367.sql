...
CREATE TABLE users (
    ...
    -- Role System
    role TEXT NOT NULL DEFAULT 'normal' CHECK (
        role IN ('normal', 'member', 'ex_member', 'leader')
    ),
    ...
    -- Promotion Tracking
    promoted_by_leader_id UUID REFERENCES users(id) ON DELETE
    SET NULL,
        -- Timestamps
        created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
        updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
...
CREATE INDEX idx_users_ex_member_payment_due ON users(ex_member_next_payment_due)
WHERE ex_member_is_active = true;
...
CREATE TABLE posts (
    ...
    city_id UUID NOT NULL REFERENCES cities(id) ON DELETE RESTRICT,
    category_id UUID REFERENCES service_categories(id) ON DELETE
    SET NULL,
        -- Post Content
        title TEXT NOT NULL,
        ...
        status TEXT NOT NULL DEFAULT 'active' CHECK (
            status IN ('active', 'pending_payment', 'ended', 'deleted')
        ),
        -- Related Transaction
        payment_transaction_id UUID REFERENCES wallet_transactions(id) ON DELETE
    SET NULL,
        -- Engagement Metrics
...
CREATE TABLE wallet_transactions (
    ...
    payment_method TEXT,
    -- 'bank_transfer', 'mobile_money', etc.
    ...
    assigned_member_id UUID REFERENCES users(id) ON DELETE
    SET NULL,
        approved_by_member_id UUID REFERENCES users(id) ON DELETE
    SET NULL,
        approved_by_leader_id UUID REFERENCES users(id) ON DELETE
    SET NULL,
        approved_at TIMESTAMP WITH TIME ZONE,
        rejection_reason TEXT,
        -- Related Records
        related_post_id UUID REFERENCES posts(id) ON DELETE
    SET NULL,
        related_report_id UUID REFERENCES member_reports(id) ON DELETE
    SET NULL,
        -- Timestamps
        created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
        updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
...
CREATE TABLE member_reports (
    ...
    reviewed_by_leader_id UUID REFERENCES users(id) ON DELETE
    SET NULL,
        reviewed_at TIMESTAMP WITH TIME ZONE,
        leader_notes TEXT,
        -- Penalty
        penalty_charged BOOLEAN DEFAULT false,
        penalty_transaction_id UUID REFERENCES wallet_transactions(id) ON DELETE
    SET NULL,
        -- Timestamps
        created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
        updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
...
CREATE TABLE subscription_history (
    ...
    action TEXT NOT NULL CHECK (
        action IN ('activated', 'renewed', 'expired', 'restored')
    ),
...
CREATE TABLE role_change_logs (
    ...
    -- User & Leader
    user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    changed_by_leader_id UUID NOT NULL REFERENCES users(id) ON DELETE
    SET NULL,
        -- Role Change
        previous_role TEXT NOT NULL CHECK (
            previous_role IN ('normal', 'member', 'ex_member', 'leader')
        ),
        new_role TEXT NOT NULL CHECK (
            new_role IN ('normal', 'member', 'ex_member', 'leader')
        ),
...
CREATE OR REPLACE FUNCTION fn_update_post_favorites_count() RETURNS TRIGGER AS $$ BEGIN IF (TG_OP = 'INSERT') THEN
UPDATE posts
...
AFTER
INSERT
    OR DELETE ON saved_posts FOR EACH ROW EXECUTE FUNCTION fn_update_post_favorites_count();
-- Function to update updated_at timestamp
CREATE OR REPLACE FUNCTION fn_update_updated_at_column() RETURNS TRIGGER AS $$ BEGIN NEW.updated_at = NOW();
RETURN NEW;
END;
$$ LANGUAGE plpgsql;
-- Apply updated_at trigger to relevant tables
CREATE TRIGGER trigger_users_updated_at BEFORE
UPDATE ON users FOR EACH ROW EXECUTE FUNCTION fn_update_updated_at_column();
CREATE TRIGGER trigger_posts_updated_at BEFORE
UPDATE ON posts FOR EACH ROW EXECUTE FUNCTION fn_update_updated_at_column();
CREATE TRIGGER trigger_cities_updated_at BEFORE
UPDATE ON cities FOR EACH ROW EXECUTE FUNCTION fn_update_updated_at_column();
CREATE TRIGGER trigger_wallet_transactions_updated_at BEFORE
UPDATE ON wallet_transactions FOR EACH ROW EXECUTE FUNCTION fn_update_updated_at_column();
CREATE TRIGGER trigger_member_reports_updated_at BEFORE
UPDATE ON member_reports FOR EACH ROW EXECUTE FUNCTION fn_update_updated_at_column();
-- Function to generate display_id for posts
CREATE OR REPLACE FUNCTION fn_generate_post_display_id() RETURNS TRIGGER AS $$
DECLARE year_str TEXT;
seq_num INTEGER;
new_display_id TEXT;
BEGIN year_str := TO_CHAR(NOW(), 'YYYY');
...
CREATE TRIGGER trigger_generate_post_display_id BEFORE
INSERT ON posts FOR EACH ROW
    WHEN (NEW.display_id IS NULL) EXECUTE FUNCTION fn_generate_post_display_id();
-- ============================================
-- SEED DATA - Sample Cities
-- ============================================
INSERT INTO cities (name, region, is_active)
VALUES ('Nouakchott', 'Nouakchott Region', true),
...
CREATE POLICY posts_select_policy ON posts FOR
SELECT USING (
        (
            paid = true
            AND status = 'active'
...
CREATE POLICY posts_insert_own ON posts FOR
INSERT WITH CHECK (user_id = auth.uid());
-- Posts: Users can update/delete their own posts
CREATE POLICY posts_update_own ON posts FOR
UPDATE USING (user_id = auth.uid());
-- Wallet transactions: Users can view their own transactions
CREATE POLICY wallet_transactions_select_own ON wallet_transactions FOR
SELECT USING (user_id = auth.uid());
-- Notifications: Users can view their own notifications
CREATE POLICY notifications_select_own ON notifications FOR
SELECT USING (user_id = auth.uid());
-- Saved posts: Users can view/manage their own saved posts
CREATE POLICY saved_posts_select_own ON saved_posts FOR
SELECT USING (user_id = auth.uid());
CREATE POLICY saved_posts_insert_own ON saved_posts FOR
INSERT WITH CHECK (user_id = auth.uid());
CREATE POLICY saved_posts_delete_own ON saved_posts FOR DELETE USING (user_id = auth.uid());
-- ============================================
